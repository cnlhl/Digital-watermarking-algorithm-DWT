...
for i = 1:4
    ...
    file = ['carrier_', int2str(i), '.jpg'];
    % read the carrier image and secret image
    image = imread(file);       % read carrier image
    secret_image = imread('secret_fudan.bmp');    % read the secret
    [M, N, C] = size(image);
    t = 50;  % threshold of the embedded watermark
    ...
    subplot(1,5,5);
    imshow(recover);      
    title('watermark recovered');
    write_name = ['carrier_embed_', int2str(i), '.jpg'];
    imwrite(carrier_embedded,write_name, 'jpg');
end
