...
for carrierIndex = 1:numel(carriers)
    ...
    v = 0.01;
    imagemark_n = imnoise(imagemark, 'gaussian', 0, v);
    water_extract_g = extract_water(imagemark_n, t);
    subplot(2, numel(carriers), (carrierIndex));
    imshow(carrier);
    title('Carrier');
    subplot(2, numel(carriers), (carrierIndex)+4);
    imshow(water_extract_g);
    title('Watermark with Noise');
    [CDR] = cdr(water, water_extract_g);
    str = strcat(num2str(v), ' variance Gaussian noise attack CDR:');
    disp(str);
    disp(CDR);
end
